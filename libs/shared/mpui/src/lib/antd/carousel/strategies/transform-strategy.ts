...
export class MpCarouselTransformStrategy extends MpCarouselBaseStrategy {
    ...
    if (this.contents.length) {
      ...
      if (this.vertical) {
        ...
        this.renderer.setStyle(
          this.slickTrackEl,
          'height',
          `${this.length * this.unitHeight}px`
        );
        this.renderer.setStyle(
          this.slickTrackEl,
          'transform',
  ...
  switch(_f: number, _t: number): Observable<void> {
    ...
    this.renderer.setStyle(
      this.slickTrackEl,
      'transition',
      `transform ${this.carouselComponent!.mpTransitionSpeed}ms ease`
    );
  ...
  private prepareVerticalContext(lastToFirst: boolean): void {
    if (lastToFirst) {
      ...
      this.renderer.setStyle(
        this.lastEl,
        'top',
        `${-this.unitHeight * this.length}px`
      );
    }
  }
