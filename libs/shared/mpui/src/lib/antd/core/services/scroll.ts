...
@Injectable()
export class MpScrollService {
  private doc: Document;

  constructor(@Inject(DOCUMENT) doc: MpSafeAny) {
    this.doc = doc;
  }
...
export function SCROLL_SERVICE_PROVIDER_FACTORY(
  doc: Document,
  scrollService: MpScrollService
): MpScrollService {
  return scrollService || new MpScrollService(doc);
}

export const SCROLL_SERVICE_PROVIDER: Provider = {
  provide: MpScrollService,
  useFactory: SCROLL_SERVICE_PROVIDER_FACTORY,
  deps: [DOCUMENT, [new Optional(), new SkipSelf(), MpScrollService]]
};
