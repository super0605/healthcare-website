...
function getRequestAnimationFrame(): typeof requestAnimationFrame {
  ...
  return prefix
    ? (window as MpSafeAny)[`${prefix}RequestAnimationFrame`]
    : requestAnimationFramePolyfill();
}
export function cancelRequestAnimationFrame(id: number): MpSafeAny {
  if (typeof window === 'undefined') {
    return null;
  }
  ...
  return prefix
    ? (
        (window as MpSafeAny)[`${prefix}CancelAnimationFrame`] ||
        (window as MpSafeAny)[`${prefix}CancelRequestAnimationFrame`]
      )
        // @ts-ignore
        .call(this, id)
    : clearTimeout(id);
