...
export class MpResizeObserverFactory {
...
@Injectable({ providedIn: 'root' })
export class MpResizeObserver implements OnDestroy {
  ...
  constructor(private mpResizeObserverFactory: MpResizeObserverFactory) {}
  ...
  private observeElement(element: Element): Subject<ResizeObserverEntry[]> {
    if (!this.observedElements.has(element)) {
      ...
      const observer = this.mpResizeObserverFactory.create(mutations =>
