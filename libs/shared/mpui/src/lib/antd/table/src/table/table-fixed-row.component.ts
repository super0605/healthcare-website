...
@Component({
  selector: 'tr[mp-table-fixed-row], tr[mpExpand]',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  template: `
    <td class="mp-disable-td ant-table-cell" #tdElement>
      <div
...
export class MpTableFixedRowComponent
  ...
  ngOnInit(): void {
    if (this.mpTableStyleService) {
      const { enableAutoMeasure$, hostWidth$ } = this.mpTableStyleService;
      enableAutoMeasure$.subscribe(this.enableAutoMeasure$);
      hostWidth$.subscribe(this.hostWidth$);
    }
  }
  ngAfterViewInit(): void {
    this.mpTableStyleService.columnCount$
