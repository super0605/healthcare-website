...
import { MpTableData } from '../table.types';

@Component({
  selector: 'mp-table-inner-scroll',
  ...
  template: `
    <div class="ant-table-content">
      <div
        *ngIf="scrollY"
        #tableHeaderElement
        [style]="headerStyleMap"
        class="ant-table-header mp-table-hide-scrollbar"
      >
        <table
          mp-table-content
          tableLayout="fixed"
...
export class MpTableInnerScrollComponent
  implements OnChanges, AfterViewInit, OnDestroy {
  @Input() data: MpTableData[] = [];
  ...
  @Input() virtualTemplate: TemplateRef<MpSafeAny> | null = null;
  @Input() virtualItemSize = 0;
  @Input() virtualMaxBufferPx = 200;
  @Input() virtualMinBufferPx = 100;
  @Input() tableMainElement: HTMLDivElement;
  @Input() virtualForTrackBy: TrackByFunction<MpTableData> = index => index;
  ...
  ngAfterViewInit(): void {
    if (this.platform.isBrowser) {
      this.ngZone.runOutsideAngular(() => {
        const scrollEvent$ = fromEvent<MouseEvent>(
          this.tableBodyElement.nativeElement,
          'scroll'
