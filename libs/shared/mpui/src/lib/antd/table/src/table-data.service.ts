...
export class MpTableDataService implements OnDestroy {
  ...
  listOfCalcOperator$ = new BehaviorSubject<
    Array<{
      key?: string;
      sortFn: MpTableSortFn | null | boolean;
      sortOrder: MpTableSortOrder;
      filterFn: MpTableFilterFn | null | boolean;
      filterValue: MpTableFilterValue;
      sortPriority: number | boolean;
    }>
  >([]);
  queryParams$: Observable<MpTableQueryParams> = combineLatest([
    ...
    map(([listOfData, listOfCalcOperator]) => {
      ...
      for (const item of listOfFilterOperator) {
        ...
        listOfDataAfterCalc = listOfDataAfterCalc.filter(data =>
          (filterFn as MpTableFilterFn)(filterValue, data)
        );
      }
      ...
      listOfDataAfterCalc.sort((record1, record2) => {
        for (const item of listOfSortOperator) {
          ...
          if (sortFn && sortOrder) {
            const compareResult = (sortFn as MpTableSortFn)(
              record1,
              record2,
              sortOrder
            );
  ...
  listOfCurrentPageData$ = this.frontPagination$.pipe(
    switchMap(pagination =>
      pagination ? this.listOfFrontEndCurrentPageData$ : this.listOfData$
    )
  );
  ...
  updateListOfData(list: MpTableData[]): void {
