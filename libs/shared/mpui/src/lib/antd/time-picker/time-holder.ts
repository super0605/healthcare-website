...
export class TimeHolder {
  ...
  get isEmpty(): boolean {
    return !(
      isNotNil(this._hours) ||
      isNotNil(this._minutes) ||
      isNotNil(this._seconds)
    );
  }
  ...
  get viewHours(): number | undefined {
    return this._use12Hours && isNotNil(this._hours)
      ? this.calculateViewHour(this._hours!)
      : this._hours;
  }
  ...
  get defaultViewHours(): number {
    ...
    return this._use12Hours && isNotNil(hours)
      ? this.calculateViewHour(hours)
      : hours;
  }
