...
export class MpAutosizeDirective implements AfterViewInit, OnDestroy, DoCheck {
  private autosize: boolean | AutoSizeType = false;
  private el: HTMLTextAreaElement | HTMLInputElement = this.elementRef
    .nativeElement;
  ...
  @Input()
  set mpAutosize(value: string | boolean | AutoSizeType) {
    const isAutoSizeType = (
      data: string | boolean | AutoSizeType
    ): data is AutoSizeType => {
      return (
        typeof data !== 'string' &&
        typeof data !== 'boolean' &&
        (!!data.maxRows || !!data.minRows)
      );
    };
  ...
  resizeToFitContent(force: boolean = false): void {
    ...
    textarea.classList.add('mp-textarea-autosize-measuring');
    textarea.placeholder = '';
    let height =
      Math.round(
        (textarea.scrollHeight - this.inputGap) / this.cachedLineHeight
      ) *
    ...
    textarea.classList.remove('mp-textarea-autosize-measuring');
