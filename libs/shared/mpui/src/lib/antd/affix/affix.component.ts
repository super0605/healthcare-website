...
import { getStyleAsText, InputNumber, shallowEqual } from '../core/util';
...
export class MpAffixComponent implements AfterViewInit, OnChanges, OnDestroy {
  @ViewChild('fixedEl', { static: true }) private fixedEl: ElementRef<
    HTMLDivElement
  >;

  @Input() mpTarget: string | Element | Window;

  @Input()
  @WithConfig<number | null>(NZ_CONFIG_COMPONENT_NAME, null)
  @InputNumber()
  mpOffsetTop: null | number;

  @Input()
  @WithConfig<number | null>(NZ_CONFIG_COMPONENT_NAME, null)
  @InputNumber()
  mpOffsetBottom: null | number;

  @Output() readonly mpChange = new EventEmitter<boolean>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    const { mpOffsetBottom, mpOffsetTop, mpTarget } = changes;

    if (mpOffsetBottom || mpOffsetTop) {
  ...
  updatePosition(e: Event): void {
    ...
    let offsetTop = this.mpOffsetTop;
    ...
    if (
      ...
      typeof this.mpOffsetBottom !== 'number'
    ) {
      offsetMode.top = true;
      offsetTop = 0;
    } else {
      offsetMode.top = typeof offsetTop === 'number';
      offsetMode.bottom = typeof this.mpOffsetBottom === 'number';
    }
    ...
    if (scrollTop >= elemOffset.top - (offsetTop as number) && offsetMode.top) {
      ...
      this.setAffixStyle(e, {
        position: 'fixed',
        bottom: targetBottomOffset + (this.mpOffsetBottom as number),
        left: targetRect.left + elemOffset.left,
        width
      });
      this.setPlaceholderStyle({
