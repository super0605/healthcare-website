...
export function DATE_HELPER_SERVICE_FACTORY(
  ...
  const i18n = injector.get(MpI18nService);
...
@Injectable({
  ...
  deps: [
    Injector,
    [new Optional(), NZ_DATE_CONFIG],
    [new Optional(), NZ_DATE_FNS_COMPATIBLE]
  ]
})
export abstract class DateHelperService {
  constructor(
    protected i18n: MpI18nService,
    @Optional() @Inject(NZ_DATE_CONFIG) protected config: MpDateConfig,
...
export class DateHelperByDateFns extends DateHelperService {
  ...
  format(date: Date, formatStr: string): string {
    ...
    return date
      ? fnsFormat(date, mergedStr, { locale: this.i18n.getDateLocale() })
      : '';
  }
