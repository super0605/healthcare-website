  ...
  template: `
      ...
      <ng-container *ngIf="isRange; else singlePanel">
        ...
        >
          <div
            ...
            [style]="datePickerService?.arrowPositionStyle"
          ></div>
          <div class="{{ prefixCls }}-panel-container">
            <div class="{{ prefixCls }}-panels">
              <ng-container
                *ngTemplateOutlet="tplRangePart; context: { partType: 'left' }"
              ></ng-container>
              <ng-container
                *ngTemplateOutlet="tplRangePart; context: { partType: 'right' }"
              ></ng-container>
            </div>
    ...
    <ng-template #tplRangePart let-partType="partType">
      <div class="{{ prefixCls }}-panel">
        <ng-container
          *ngTemplateOutlet="tplInnerPopup; context: { partType: partType }"
        ></ng-container>
      </div>
    </ng-template>
...
export class DateRangePopupComponent implements OnInit, OnChanges, OnDestroy {
  ...
  onClickOk(): void {
    if (this.datePickerService.hasOnePart()) {
      const otherPart =
        this.datePickerService.activeInput === 'left' ? 'right' : 'left';
  ...
  onSelectTime(value: CandyDate, partType?: RangePartType): void {
    if (this.isRange) {
      ...
      const newValue = this.overrideHms(value, this.datePickerService
        .value as CandyDate);
  ...
  enablePrevNext(
    ...
    if (this.isRange) {
      const [start, end] = normalizeRangeValue(this.datePickerService
        .activeDate as CandyDate[]);
  ...
  private buildTimeOptions(): void {
    if (this.showTime) {
      ...
      if (this.isRange) {
        ...
        this.timeOptions = this.overrideTimeOptions(showTime, this
          .datePickerService.value as CandyDate);
      }
    } else {
