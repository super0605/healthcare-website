...
import { FunctionProp, MpSafeAny } from '../../core/types';
...
export abstract class AbstractTable implements OnInit, OnChanges {
  ...
  trackByBodyRow(_index: number, item: DateBodyRow): MpSafeAny {
    return item;
  }

  // Item usually is an object, so trackby has no use by default.
  trackByBodyColumn(_index: number, item: DateCell): MpSafeAny {
    return item;
  }
