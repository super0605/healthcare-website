...
import { CandyDate, CompatibleValue } from '../core/time';
...
@Component({
  ...
  template: `
    ...
    <ng-template #tplRightRest>
      ...
      >
        <i mp-icon mpType="close-circle" mpTheme="fill"></i>
      </span>
      <span class="{{ prefixCls }}-suffix">
        <i mp-icon mpType="calendar"></i>
      </span>
    </ng-template>
...
export class MpPickerComponent
  ...
  ngAfterViewInit(): void {
    ...
    if (this.isRange) {
      ...
      const arrowLeft =
        inputWidth + this.separatorElement.nativeElement.offsetWidth;
  ...
  // All other components like "mp-dropdown" which depends on overlay also has the same issue.
  // See: https://github.com/NG-ZORRO/ng-zorro-antd/issues/1429
  onPositionChange(position: ConnectedOverlayPositionChange): void {
    this.dropdownAnimation =
      position.connectionPair.originY === 'top' ? 'top' : 'bottom';
  ...
  updateInputValue(returnToInit: boolean = false): void {
    const newValue = returnToInit
      ? this.datePickerService.initialValue
  ...
  private checkValidInputDate(inputTarget: EventTarget): CandyDate | null {
    ...
    if (
      !date.isValid() ||
      input !== this.dateHelper.format(date.nativeDate, this.format)
    ) {
