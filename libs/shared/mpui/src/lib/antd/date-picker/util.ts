...
const defaultDisabledTime: DisabledTimeConfig = {
  ...
  mpDisabledMinutes(): number[] {
    return [];
  },
  mpDisabledSeconds(): number[] {
    return [];
  }
};
...
export function isTimeValidByConfig(
  ...
  if (value) {
    ...
    const disabledHours = disabledTimeConfig.mpDisabledHours();
    if (disabledHours.indexOf(hour) === -1) {
      const disabledMinutes = disabledTimeConfig.mpDisabledMinutes(hour);
      if (disabledMinutes.indexOf(minutes) === -1) {
        const disabledSeconds = disabledTimeConfig.mpDisabledSeconds(
          hour,
          minutes
        );
