...
import { MpTabPositionMode } from './table.types';

@Directive({
  selector: '[mp-tabs-ink-bar]',
  exportAs: 'mpTabsInkBar',
  host: {
    '[class.ant-tabs-ink-bar-animated]': 'mpAnimated',
    '[class.ant-tabs-ink-bar-no-animated]': '!mpAnimated'
  }
})
export class MpTabsInkBarDirective {
  @Input() @InputBoolean() mpAnimated = false;

  @Input() mpPositionMode: MpTabPositionMode = 'horizontal';
