...
@Component({
  selector: 'mp-pagination-default',
...
export class MpPaginationDefaultComponent implements OnChanges {
  ...
  ranges = [0, 0];
  listOfPageItem: Array<Partial<MpPaginationItemComponent>> = [];
  ...
  trackByPageItem(
    _: number,
    value: Partial<MpPaginationItemComponent>
  ): string {
  ...
  getListOfPageItem(
    pageIndex: number,
    lastIndex: number
  ): Array<Partial<MpPaginationItemComponent>> {
    const concatWithPrevNext = (
      listOfPage: Array<Partial<MpPaginationItemComponent>>
    ) => {
      const prevItem = {
        type: 'prev',
