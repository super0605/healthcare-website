...
import { IconConfig, MpConfigService } from '../core/config';
import { warn } from '../core/logger';
import { MpSafeAny } from '../core/types';
import { Subject } from 'rxjs';

export interface MpIconfontOption {
  scriptUrl: string;
}

export const NZ_ICONS = new InjectionToken('mp_icons');
export const NZ_ICON_DEFAULT_TWOTONE_COLOR = new InjectionToken(
  'mp_icon_default_twotone_color'
);
...
export class MpIconService extends IconService {
  ...
  fetchFromIconfont(opt: MpIconfontOption): void {
    const { scriptUrl } = opt;
  ...
  constructor(
    ...
    @Optional() @Inject(DOCUMENT) _document: MpSafeAny,
  ...
  private configDefaultTheme(): void {
    ...
    this.defaultTheme = iconConfig.mpTheme || 'outline';
  }

  private configDefaultTwotoneColor(): void {
    const iconConfig = this.getConfig();
    const defaultTwotoneColor =
      iconConfig.mpTwotoneColor || DEFAULT_TWOTONE_COLOR;
  ...
  private getConfig(): IconConfig {
    return this.mpConfigService.getConfigForComponent('icon') || {};
  }
}

export const NZ_ICONS_PATCH = new InjectionToken('mp_icons_patch');

@Injectable()
export class MpIconPatchService {
  patched = false;

  constructor(
    @Self() @Inject(NZ_ICONS_PATCH) private extraIcons: IconDefinition[],
    private rootIconService: MpIconService
  ) {}
  ...
  doPatch(): void {
    ...
    this.extraIcons.forEach(iconDefinition =>
      this.rootIconService.addIcon(iconDefinition)
    );
    this.patched = true;
  }
}
