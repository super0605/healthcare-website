...
import { MpIconService } from '../icon';
...
@Component({
  ...
  template: `
    ...
    <div class="icon-selector">
      ...
      >
        <label mp-radio-button mpValue="outline">
          <i mp-icon>
            <svg>
              <path
        ...
        <label mp-radio-button mpValue="fill">
          <i mp-icon>
            <svg>
              <path
        ...
        <label mp-radio-button mpValue="twotone">
          <i mp-icon>
            <svg>
              <path
      ...
      <mp-input-group [mpSuffix]="suffixIconSearch">
        <input
          mp-input
    ...
    <ng-container *ngFor="let category of categoryNames; let i = index">
      ...
      <ul class="anticons-list">
        ...
        >
          <i mp-icon [mpType]="icon" [mpTheme]="currentTheme"></i>
          <span class="anticon-class">
            <mp-badge *ngIf="isNewIcon(icon); else notNewTpl" mpDot>
              {{ icon }}
            </mp-badge>
...
export class MpPageDemoIconComponent implements OnInit {
  ...
  onIconClick(e: MouseEvent, icon: string): void {
    ...
    const copiedString = `<i mp-icon mpType="${icon}" mpTheme="${this.currentTheme}"></i>`;
  ...
  prepareIcons(): void {
    ...
    let notEmptyCategories = Object.keys(categories).map(category => ({
      ...
      icons: categories[category].filter(
        (name: string) =>
          currentThemeIcons.indexOf(name) > -1 &&
          name.includes(this.searchingString)
      )
    }));
    ...
    notEmptyCategories = notEmptyCategories.filter(({ icons }) =>
  ...
  constructor(
    ...
    private _iconService: MpIconService
  ) {
