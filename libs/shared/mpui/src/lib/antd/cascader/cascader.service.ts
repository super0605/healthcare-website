...
/**
 * All data is stored and parsed in MpCascaderService.
 */
@Injectable()
export class MpCascaderService implements OnDestroy {
  /** Activated options in each column. */
  activatedOptions: MpCascaderOption[] = [];

  /** An array to store cascader items arranged in different layers. */
  columns: MpCascaderOption[][] = [];

  /** If user has entered searching mode. */
  inSearchingMode = false;

  /** Selected options would be output to user. */
  selectedOptions: MpCascaderOption[] = [];
  ...
  private columnsSnapshot: MpCascaderOption[][] = [[]];

  /** To hold activated options before entering searching mode. */
  private activatedOptionsSnapshot: MpCascaderOption[] = [];

  private cascaderComponent: MpCascaderComponentAsSource;

  /** Return cascader options in the first layer. */
  get mpOptions(): MpCascaderOption[] {
  ...
  syncOptions(first: boolean = false): void {
    ...
    const initColumnWithIndex = (columnIndex: number) => {
      const activatedOptionSetter = () => {
        ...
        const option =
          ...
          (typeof currentValue === 'object'
            ? currentValue
            : {
                [`${this.cascaderComponent.mpValueProperty}`]: currentValue,
                [`${this.cascaderComponent.mpLabelProperty}`]: currentValue
              });
      ...
      if (this.isLoaded(columnIndex) || !this.cascaderComponent.mpLoadData) {
    ...
    if (first && this.cascaderComponent.mpLoadData && !hasValue) {
   ...
   */
  withComponent(cascaderComponent: MpCascaderComponentAsSource): void {
   ...
   */
  withOptions(options: MpCascaderOption[] | null): void {
  ...
  setOptionSelected(option: MpCascaderOption, index: number): void {
    const changeOn = this.cascaderComponent.mpChangeOn;
    ...
    if (
      option.isLeaf ||
      this.cascaderComponent.mpChangeOnSelect ||
   ...
   * @param option
   */
  setSearchOptionSelected(option: MpCascaderSearchOption): void {
  ...
  prepareSearchOptions(searchValue: string): void {
    const results: MpCascaderOption[] = []; // Search results only have one layer.
    const path: MpCascaderOption[] = [];
    const defaultFilter: MpCascaderFilter = (i, p) => {
      return p.some(o => {
        const label = this.getOptionLabel(o);
        return !!label && label.indexOf(i) !== -1;
      });
    };
    const showSearch = this.cascaderComponent.mpShowSearch;
    const filter =
    ...
    const sorter =
      isShowSearchObject(showSearch) && showSearch.sorter
        ? showSearch.sorter
        : null;
    const loopChild = (node: MpCascaderOption, forceDisabled = false) => {
      path.push(node);
      const cPath = Array.from(path);
      if (filter(searchValue, cPath)) {
        const disabled = forceDisabled || node.disabled;
        const option: MpCascaderSearchOption = {
          disabled,
          isLeaf: true,
          path: cPath,
          [this.cascaderComponent.mpLabelProperty]: cPath
    ...
    const loopParent = (node: MpCascaderOption, forceDisabled = false) => {
      const disabled = forceDisabled || node.disabled;
      path.push(node);
  ...
  getOptionLabel(o: MpCascaderOption): string {
    return o[this.cascaderComponent.mpLabelProperty || 'label'] as string;
  }

  getOptionValue(o: MpCascaderOption): MpSafeAny {
    return o[this.cascaderComponent.mpValueProperty || 'value'];
  }
   ...
   */
  loadChildren(
    option: MpCascaderOption | MpSafeAny,
    columnIndex: number,
    success?: VoidFunction,
    failure?: VoidFunction
  ): void {
    const loadFn = this.cascaderComponent.mpLoadData;
  ...
  private findOptionWithValue(
    columnIndex: number,
    value: MpCascaderOption | MpSafeAny
  ): MpCascaderOption | null {
