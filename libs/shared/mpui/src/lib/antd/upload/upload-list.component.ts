...
import { NgClassType, MpSafeAny } from '../core/types';
...
@Component({
  selector: 'mp-upload-list',
  exportAs: 'mpUploadList',
...
export class MpUploadListComponent implements OnChanges {
  list: UploadListFile[];
  hostClassMap = {};
  ...
  isImageUrl(file: UploadFile): boolean {
    ...
    if (
      /^data:image\//.test(url) ||
      /(webp|svg|png|gif|jpg|jpeg|jfif|bmp|dpg)$/i.test(extension)
    ) {
      return true;
