...
@Component({
  selector: '[mp-upload-btn]',
  exportAs: 'mpUploadBtn',
...
export class MpUploadBtnComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private traverseFileTree(files: DataTransferItemList): void {
    const _traverseFileTree = (item: MpSafeAny, path: string) => {
      if (item.isFile) {
        item.file((file: File) => {
        ...
        dirReader.readEntries((entries: MpSafeAny) => {
    ...
    for (const file of files as MpSafeAny) {
  ...
  uploadFiles(fileList: FileList | File[]): void {
    let filters$: Observable<UploadFile[]> = of(
  ...
  private post(file: UploadFile): void {
    ...
    if (typeof action === 'function') {
      const actionResult = (action as (
        file: UploadFile
      ) => string | Observable<string>)(file);
    ...
    if (typeof transformFile === 'function') {
      ...
      process$ = process$.pipe(
        switchMap(() =>
          transformResult instanceof Observable
            ? transformResult
            : of(transformResult)
        )
      );
    ...
    if (typeof data === 'function') {
      const dataResult = (data as (file: UploadFile) => {} | Observable<{}>)(
        file
      );
    ...
    if (typeof headers === 'function') {
      const headersResult = (headers as (
        file: UploadFile
      ) => {} | Observable<{}>)(file);
    ...
    process$.subscribe(newFile => {
      ...
      if (!(req$ instanceof Subscription)) {
        warn(`Must return Subscription type in '[mpCustomRequest]' property`);
      }
      this.reqs[uid] = req$;
      opt.onStart!(file);
    });
  ...
  private xhr(args: UploadXHRArgs): Subscription {
    ...
    formData.append(args.name!, args.postFile as MpSafeAny);
    ...
    return this.http.request(req).subscribe(
      (event: HttpEvent<MpSafeAny>) => {
        if (event.type === HttpEventType.UploadProgress) {
          if (event.total! > 0) {
            (event as MpSafeAny).percent = (event.loaded / event.total!) * 100;
          }
