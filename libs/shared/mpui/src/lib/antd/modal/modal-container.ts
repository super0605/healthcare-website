...
import { getElementOffset } from '../core/util';

import { MpModalRef } from './modal-ref';
import { ModalOptions } from './modal-types';

export function throwMpModalContentAlreadyAttachedError(): never {
  throw Error(
...
export class BaseModalContainer extends BasePortalOutlet {
  ...
  constructor(
    ...
    this.isStringContent = typeof config.mpContent === 'string';
    this.setContainer();
  }
  ...
  onMouseup(e: MouseEvent): void {
    if (
      e.target === this.latestMousedownTarget &&
  ...
  attachComponentPortal<T>(portal: ComponentPortal<T>): ComponentRef<T> {
    if (this.portalOutlet.hasAttached()) {
      throwMpModalContentAlreadyAttachedError();
    }
  ...
  private animationDisabled(): boolean {
    return this.config.mpNoAnimation || this.animationType === 'NoopAnimations';
  }
  ...
  private setModalTransformOrigin(): void {
    ...
    if (this.elementFocusedBeforeModalWasOpened as HTMLElement) {
      ...
      const transformOrigin = `${x - modalElement.offsetLeft}px ${y -
  ...
  private savePreviouslyFocusedElement(): void {
    if (this.document) {
      this.elementFocusedBeforeModalWasOpened = this.document
  ...
  private bindBackdropStyle(): void {
    this.zone.runOutsideAngular(() => {
      ...
      if (
        typeof this.config.mpMaskStyle === 'object' &&
        Object.keys(this.config.mpMaskStyle).length
      ) {
        const backdropElement = this.overlayRef.backdropElement;
        const styles: { [key: string]: string } = {
          ...this.config.mpMaskStyle
        };
        Object.keys(styles).forEach(key => {
  ...
  private getContainer(): HTMLElement | null {
    const { mpGetContainer } = this.config;
    const container =
      typeof mpGetContainer === 'function' ? mpGetContainer() : mpGetContainer;
